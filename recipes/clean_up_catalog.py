...
if __name__ == "__main__":
    ...
    band = sys.argv[1]
    mmag = dict(g=15, z=15, u=12).get(band, 15)
    in_file = sys.argv[2]
    gsc_file = sys.argv[3]
    out_file = sys.argv[4]
    out_reg = os.path.splitext(out_file)[0] + '.reg'

    pad = 40  # number of pixels to edge
    ...
    cdec = np.mean(cat['DELTA_J2000'])
    gsc = gsc[(gsc['ra'] > roi[0] - roipad / np.cos(cdec * np.pi / 180.)) &
              (gsc['ra'] < roi[1] + roipad / np.cos(cdec * np.pi / 180.)) &
              (gsc['dec'] > roi[2] - roipad) &
              (gsc['dec'] < roi[3] + roipad)
              ]
